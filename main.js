...
for (const coffee of coffees) {
    if (coffee.includes("light")) {
        output += `I'll have the ${"light colombian roast"} and drink it black`
    }
    else if (coffee.includes("medium")) {
        output += `I'll have the ${"guatemalan blend medium roast"} and add cream only`
    }
    else if (coffee.includes("dark")) {
        output += `I'll have the ${"dark madagascar blend"} and add cream and sugar`
    }
    output += "\n"
}
